  ...
  return (
    <>
      ...
      <section id="services" className="site-container scroll-mt-24 py-20 md:py-32">
        ...
        <h3 className="mt-16 text-[clamp(2.25rem,5.2vw,5.5rem)] leading-[0.95] font-normal tracking-[-0.04em] text-ink uppercase md:mt-24">
        ...
        {data.categories.map((cat) => {
          ...
          return (
            <section
              ...
              <h2
                id={`category-${cat.id}`}
                className="col-span-12 text-[clamp(2.25rem,5.2vw,5.5rem)] leading-[0.95] font-normal tracking-[-0.04em] break-words text-ink uppercase">
                <MaskLine>({cat.title})</MaskLine>
              </h2>

              <div className="col-span-12 self-start lg:sticky lg:top-32 lg:col-span-5">
                <Reveal>
                  <p className="max-w-md text-base leading-[1.7] text-ink">
                    Professional {cat.title.toLowerCase()} services designed to seamlessly integrate
                    with your core operations.
                  </p>
                </Reveal>
                <Reveal delay={0.08}>
                  <p className="mt-5 max-w-md text-base leading-[1.7] text-clay">{copy.intro}</p>
                </Reveal>
                <Reveal delay={0.16}>
                  <ul className="mt-8 max-w-md border-b border-ink/10">
                    {copy.points.map((point) => (
                      <li key={point} className="border-t border-ink/10 py-3 text-[15px] leading-snug text-ink">
                        {point}
                      </li>
                    ))}
                  </ul>
                  ...
                  <li key={service.id}>
                    <Link href="/contact" className="group block border-t border-ink/10">
                      <Reveal
                        ...
                        <h3 className="col-span-12 text-2xl leading-[1.08] font-normal tracking-[-0.025em] break-words text-ink uppercase transition-colors duration-500 group-hover:text-clay md:col-span-9 md:text-[1.875rem]">
                          {service.title}
                        </h3>
