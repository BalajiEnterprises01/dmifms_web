      ...
      <div className="grid grid-cols-12 gap-x-6 gap-y-10">
        <div className="col-span-12 grid grid-cols-12 gap-x-6 gap-y-4 self-start lg:sticky lg:top-32 lg:col-span-5">
          <Reveal className="col-span-12 md:col-span-3 lg:col-span-5">
            <h2 className="text-lg font-semibold text-ink">Why DM23</h2>
          </Reveal>
          <Reveal delay={0.08} className="col-span-12 md:col-span-6 lg:col-span-7">
            <p className="max-w-sm text-body leading-[1.7] text-clay">{intro}</p>
          </Reveal>
        </div>
          ...
          {items.map((item) => (
            <li key={item.id}>
              <Reveal className="grid grid-cols-12 gap-x-6 gap-y-3 py-8 md:py-11">
                <h3 className="col-span-12 text-2xl leading-[1.08] font-normal tracking-[-0.025em] text-ink uppercase md:col-span-6 md:text-row">
                  ({item.title})
                </h3>
                <p className="col-span-12 max-w-md text-body leading-[1.7] text-clay md:col-span-6">
                  {item.description}
                </p>
              </Reveal>
              <Rule />
